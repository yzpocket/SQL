...
--[실습]
--emp테이블에서 모든 SALESMAN에 대하여 급여의 평균, 최고액,최저액,합계, 인원수를 구하여 출력하세요.
...
#그룹 함수는 GROUP BY 절과 함께 사용 된다.
emp에서 업무별(job단위로 묶어서라는 뜻) 인원수를 보여주세요.
select job, count(empno) from emp
group by job;

#GROUP BY절을 이용 할 때는 GROUP BY절에서 사용한 컬럼과 그룹함수만 SELECT 할 수 있다.
...
select * from products;
select ep_code_fk, trunc(avg(input_price))
from products
group by ep_code_fk;
...
#WGHO 순서 where,group by,having, order by

#HAVING 절
- GROUP BY의 결과에 조건을 주어 제한 할 때 사용 한다.
